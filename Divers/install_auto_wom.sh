...
############################
# CONFIGURATION CLAVIER FR #
############################
echo "=== Configuration du clavier en français ==="
# Installation du package si nécessaire
install_if_missing console-setup
# Configuration persistante du clavier pour la console
...
### REDIRECTION DES PORTS POUR CAMERAS
### Port par défaut des caméras
LAN_PORT_BASIC=9000
LAN_PORT_RTMP=1935
LAN_PORT_HTTP=80
LAN_PORT_HTTPS=443
...
for ((i=0; i<CAM_COUNT; i++)); do
    ...
    iptables -t nat -A PREROUTING -i "$VPN_IF" -m addrtype --dst-type LOCAL -p tcp --dport $((WAN_PORT_BASIC + i)) -j DNAT --to "${CAM_IP}:${LAN_PORT_BASIC}"
    iptables -t nat -A PREROUTING -i "$VPN_IF" -m addrtype --dst-type LOCAL -p tcp --dport $((WAN_PORT_RTMP + i)) -j DNAT --to "${CAM_IP}:${LAN_PORT_RTMP}"
    iptables -t nat -A PREROUTING -i "$VPN_IF" -m addrtype --dst-type LOCAL -p tcp --dport $((WAN_PORT_HTTP + i)) -j DNAT --to "${CAM_IP}:${LAN_PORT_HTTP}"
    iptables -t nat -A PREROUTING -i "$VPN_IF" -m addrtype --dst-type LOCAL -p tcp --dport $(((i+1)*1000 + 443)) -j DNAT --to "${CAM_IP}:${LAN_PORT_HTTPS}"
    iptables -t nat -A PREROUTING -i "$VPN_IF" -m addrtype --dst-type LOCAL -p tcp --dport $((WAN_PORT_RTSP + i)) -j DNAT --to "${CAM_IP}:${LAN_PORT_RTSP}"
    iptables -t nat -A PREROUTING -i "$VPN_IF" -m addrtype --dst-type LOCAL -p tcp --dport $((WAN_PORT_ONVIF + i)) -j DNAT --to "${CAM_IP}:${LAN_PORT_ONVIF}"
done

########################
# FORWARD (VIDÉO SAFE) #
########################
### Autorisation flux déjà établis / retours
...
### Bloquage sortie LAN directe vers le WAN
...
### Désactivation des helpers automatiques (souvent problématiques pour CCTV)
...
###################################
# INSTALL FFMPEG X265 IDÉMPOTENT #
###################################
echo "=== Installation des dépendances pour compilation ffmpeg ==="
grep -Eq "^[^#]*\bnon-free\b" /etc/apt/sources.list || \
echo "deb http://deb.debian.org/debian trixie main contrib non-free non-free-firmware" >> /etc/apt/sources.list
apt update
install_if_missing autoconf automake build-essential cmake git-core libass-dev libfreetype6-dev \
...
libnuma-dev libvpx-dev libfdk-aac-dev libmp3lame-dev ccache

mkdir -p "$FFMPEG_SRC_DIR"
cd "$FFMPEG_SRC_DIR"

### Env pour pkg-config et ccache
...
compile_x265() {
    ...
    [ ! -d "$FFMPEG_SRC_DIR/x265" ] && git clone https://bitbucket.org/multicoreware/x265_git x265
    cd x265/build/linux
...
compile_ffmpeg() {
    if command -v ffmpeg &>/dev/null && ffmpeg -encoders | grep -q x265; then
        echo "FFmpeg avec x265 déjà installé, compilation ignorée."
        return
    fi
    echo "=== Compilation FFmpeg ==="
    [ ! -d "$FFMPEG_SRC_DIR/ffmpeg" ] && git clone https://git.ffmpeg.org/ffmpeg.git ffmpeg
...
###################################
echo "=== Création services et timers systemd ==="
create_timer() {
    SERVICE=$1
